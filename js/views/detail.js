async function loadDetails() {
  ...
  const mFromStore = await crmDb.get('metrics', currentCIF);
  const cached = mFromStore || customer?.metrics;
  ...
  if (navigator.onLine) {
    ...
    try {
      ...
      const idx = customers.findIndex(c => c.id === currentCIF);
      if (idx !== -1) customers[idx].metrics = m;
      renderMetrics(m);
    } catch (err) {
      clearTimeout(timeoutId);
      toggleLoader(false);
